...
export default function DiningClient({
  ...
  initialRestaurants?: DiningItem[];
  initialSettings?: HotelSettings | null;
}) {
  const pageRef = useRef<HTMLDivElement>(null);
  const [restaurants, setRestaurants] = useState<DiningItem[]>(initialRestaurants);
  const [settings, setSettings] = useState<HotelSettings | null>(initialSettings);
  const [isLoading, setIsLoading] = useState(initialRestaurants.length === 0);

  useEffect(() => {
    if (restaurants.length > 0 && settings) {
      setIsLoading(false);
      return;
    }
    let isMounted = true;
    Promise.allSettled([getDiningVenues(), getSettings()])
      .then(([venuesRes, settingsRes]) => {
        if (!isMounted) return;
        if (venuesRes.status === "fulfilled" && venuesRes.value?.length) setRestaurants(venuesRes.value);
