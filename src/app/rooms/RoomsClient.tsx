...
function RoomsPageInner({
  ...
  const [isLoading, setIsLoading] = useState(initialRooms.length === 0);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([getRoomTypes(), getSettings()])
      .then(([roomsRes, settingsRes]) => {
        if (!isMounted) return;
        if (roomsRes.status === "fulfilled" && roomsRes.value?.length) setRooms(roomsRes.value);
...
export default function RoomsClient({
  ...
  return (
    <Suspense fallback={<div className="min-h-screen bg-white" />}>
      <RoomsPageInner initialRooms={initialRooms} initialSettings={initialSettings} />
    </Suspense>
  );
}
