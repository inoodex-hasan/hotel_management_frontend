...
export default function AboutClient({
  ...
  const [dining, setDining] = useState<DiningItem[]>(initialDining);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (diningRes.status === "fulfilled" && diningRes.value?.length) setDining(diningRes.value);
      if (facRes.status === "fulfilled" && facRes.value?.length) setFacilities(facRes.value);
      setIsLoading(false);
    });
    return () => {
      isMounted = false;
